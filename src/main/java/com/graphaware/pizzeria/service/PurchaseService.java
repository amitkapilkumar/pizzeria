...
public class PurchaseService {
    ...
    private Double computeAmount(List<Pizza> pizzas) {
        ...
        boolean applyPineappleDiscount = pizzas.stream().anyMatch(pizza -> pizza.getToppings().contains("pineapple"));

        // buy 3 get cheapest free
        boolean applyBuy3GetCheapestFree = pizzas.size() == 3;

        if(applyPineappleDiscount) {
            return Double.valueOf(
                    new DecimalFormat("#.##").format(pizzas.stream()
                            .mapToDouble(pizza -> pizza.getToppings().contains("pineapple")? pizza.getPrice() : pizza.getPrice() * 0.9).sum()));
        }

        if(applyBuy3GetCheapestFree) {
            double price = pizzas.stream().mapToDouble(Pizza::getPrice).sum() - pizzas.stream().mapToDouble(Pizza::getPrice).min().getAsDouble();
            return Double.valueOf(new DecimalFormat("#.##").format(price));
        }

        return Double.valueOf(new DecimalFormat("#.##").format(pizzas.stream().mapToDouble(Pizza::getPrice).sum()));
    }
